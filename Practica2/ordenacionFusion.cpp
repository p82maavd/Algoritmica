...
void fusion(vector<int> &t, int n){
	...
	if(t.size()<n){ /*Ordena usando un metodo no sofisticado */
...
void tiemposOrdenacionExtraFusion(int nMin, int nMax, int n, int repeticiones, int incremento, vector <double> &tiemposReales, vector <double> &numeroElementos){
...
void ordenacionExtraFusion(){
	...
	cout<<"Introduce el tamaño minimo del vector para ordenarlo por Fusion (El optimo es sobre "<<calculoOptimoFusion()<<"): ";
	cin>>rango;

	vector<double> tiemposReales(0), numeroElementos(0), a(2);
	tiemposOrdenacionExtraFusion(min,max,rango,iter,incremento,tiemposReales,numeroElementos);
	if(numeroElementos.size()==1){
		cout << numeroElementos[0] << " elementos ordenados en " << tiemposReales[0] << " ms"<<endl;
	}
	ajusteLogaritmico(numeroElementos, tiemposReales, a);
